  ...
  const Topics = [
    ...
    },
    {
      ...
      process: [
        ...
        <b>E.g. </b>,
        <br />,
        'Let the given equation be 2x² + 8x + 6 = 0',
        <br />,
        'First root = (-b + √ (b² - 4ac) )/2a = -1',
        <br />,
        'Second root = (-b - √ (b² - 4ac) )/2a = -0.25 - 53.07i  where i = √-1',
        <br />,
      ],
    },
  ];
  ...
  const Statistics = () => {
    ...
    return (
      <>
        ...
        <div className='formula-table mb-4' style={{ marginTop: '50px' }}>
  ...
  const Quadratic = () => {
    ...
    const calcRoots = () => {
      ...
      if (d < 0) {
        d = -d;
        setroot1(
          -b / (2 * a) + ' + ' + ((Math.sqrt(d) / 2) * a).toFixed(2) + 'i'
        );
        setroot2(
          -b / (2 * a) + ' - ' + ((Math.sqrt(d) / 2) * a).toFixed(2) + 'i'
        );
      } else {
        setroot1((-b + Math.sqrt(d)) / (2 * a)).toFixed(2);
        setroot2((-b - Math.sqrt(d)) / (2 * a)).toFixed(2);
      }
      console.log(root1);
    };
    return (
      <>
        <Form>
          ...
          <Form.Group className='mb-4'>
            <Form.Label>Roots of the equation are</Form.Label>
            <Form.Control
              readOnly
              type='number'
              placeholder={
                root1 === null ? 'First root' : root1 + ' and ' + root2
              }
            />
          </Form.Group>
        </Form>
        <div className='button-custom-grp'>
          <Button variant='primary' onClick={calcRoots}>
            Calculate
          </Button>
          &nbsp;&nbsp;&nbsp;
